...
class Storage(BaseObject):
    ...
    class ReadSettingsContext(BaseObject):
        def __init__(self) -> None:
            ...
            self.fallbackConfigLegacyDcOptions : td.MTP.DcOptions = td.MTP.DcOptions(td.MTP.Environment.Production)
    ...
    def PrepareEncrypted(data : EncryptedDescriptor, key : td.AuthKey) -> QByteArray:
        ...
        encrypted = encrypted[:0x10] + Storage.aesEncryptLocal(toEncrypt, key, encrypted)
    ...
    def ReadFile(fileName : str, basePath : str) -> FileReadDescriptor:
        ...
        for chr in to_try:
            try:
                ...
                if magic != TDF_MAGIC:
                    tries_exception = TDataInvalidMagic("Invalid magic {magic} in file {fileName}")
                    file.close()
                    continue
                ...
                if (check_md5.hexdigest() != md5.hex()):
                    tries_exception = TDataInvalidCheckSum("Invalid checksum {check_md5.hexdigest()} in file {fileName}")
                    file.close()
                    continue
    ...
    def CreateLocalKey(salt : QByteArray, passcode: QByteArray = QByteArray()) -> td.AuthKey:
        ...
        return td.AuthKey(hashlib.pbkdf2_hmac("sha512", hashKey.digest(), salt, iterationsCount, 256))

    @staticmethod
    def CreateLegacyLocalKey(salt : QByteArray, passcode: QByteArray = QByteArray()) -> td.AuthKey:

        iterationsCount = 1 if passcode.isEmpty() else 100000
        return td.AuthKey(hashlib.pbkdf2_hmac("sha512", passcode.data(), salt.data(), iterationsCount, 256))

    @staticmethod
    def aesEncryptLocal(src : QByteArray, authKey : td.AuthKey, key128 : QByteArray) -> QByteArray:
    ...
    @staticmethod   
    def aesDecryptLocal(src : QByteArray, authKey : td.AuthKey, key128 : QByteArray) -> QByteArray:
        aesKey, aesIv = authKey.prepareAES_oldmtp(key128, False)
    ...
    def DecryptLocal(encrypted : QByteArray, authKey : td.AuthKey) -> EncryptedDescriptor:
        ...
        if (checkHash != encryptedKey):
            raise TDataBadDecryptKey("Bad decrypt key, data not decrypted - incorrect password?")
        ...
        if (dataLen > decrypted.size()) or (dataLen <= fullLen - 16) or (dataLen < 4):
            raise TDataBadDecryptedDataSize("Bad decrypted part size: {encryptedSize}, fullLen: {fullLen}, decrypted size: {decrypted.__len__()}")
