...
class BaseMetaClass(abc.ABCMeta):
    ...
    def __new__(cls : Type[_T], clsName : str, bases : List[object], attrs : Dict[str, Any]) -> _T:
        ...
        if debug.IS_DEBUG_MODE:
            ignore_list = ["__new__", "__get__", "__call__", "__set_name__", "__str__", "__repr__"]

            for attr, val in attrs.items():
                if not attr in ignore_list and callable(val) and not isinstance(val, type):
...
class override(object):
    ...
    def isOverride(func : _F) ->bool:
        if not hasattr(func, "__isOverride__"): return False
...
class extend_class(object):
    ...
    def __new__(cls, decorated_cls : _TCLS, isOverride : bool = False) -> _TCLS:
        ...
        [(newAttributes.pop(cross) if cross in newAttributes else None) for cross in crossDelete]
                ...
                if result != None:
                    if id(result['value']) == id(attributeValue):
                        ...
                        if not override.isOverride(attributeValue):
                            print(f"[{attributeName}] {id(result['value'])} - {id(attributeValue)}")
    ...
    def object_hierarchy_getattr(obj : object, attributeName : str) -> List[str]:
        ...
        results = []
        if type(obj) == object: return results
