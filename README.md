...
- [x] Develop opentele-tui using [textual](https://github.com/Textualize/textual) for non-experience user.
...
## opentele-tui (Beginner-friendly terminal UI)

If you prefer a guided interface instead of writing Python scripts, install and run the TUI:

```bash
pip install "opentele[tui]"
opentele-tui
```

The interface supports both directions:
- `tdata` folder → `telethon` session file
- `telethon` session file → `tdata` folder
